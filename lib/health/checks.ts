/**
 * System health checks - extracted from the /settings/health page so the
 * daily alert cron can run the same checks and email staff when any are
 * in 'error' state. Page + cron stay in sync because they share this code.
 */
...
export async function runChecks(shopId: string): Promise<Check[]> {
